...
const url = "https://reqres.in/api/users?page=2"; // на page 2 - id c 7 по 12
...
async function getUserData(ID) {
    try {
        ...
        if (response.ok) {
            ...
            for (let i = 0; i < pageUsers["data"].length; i++) {
                if (pageUsers["data"][i]["id"] === ID) {
                    return console.log(pageUsers["data"][i]);
                }
            }
            return console.log(`Ошибка! Пользователь с id: ${ID} - не найден!`);
        } else {
